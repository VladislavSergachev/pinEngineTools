...
int main()
{
   ...
   strcpy(pathToSave, DEFAULT_ANIM_NAME);
    ...
    prTexture.loadFromFile("default.png");
    ...
    while(mainWin.isOpen())
    {
            ...
            if(ImGui::BeginMenuBar())
            {
                if(ImGui::BeginMenu("File"))
                {
                    if(ImGui::BeginMenu("Open"))
                    {
                        ...
                        {
                            mode = DialogMode::OpenImage;
                            procImgFiles();
                            printf("%d\n", 88);
                        }
                        if(ImGui::MenuItem("Animation"))
                        {
                            mode = DialogMode::OpenAnim;
                            procAnimFiles();
                            printf("%d\n", 89);
                        }
                        ImGui::EndMenu();
                    ...
                    {
                        mode = DialogMode::SaveAnimToFile;
                    }
                    #ifdef CONVERT_FEAUTURE
                    if(ImGui::MenuItem("Convert"))
                    {
                        mode = DialogMode.ConvertToCPP;
                        procAnimFiles();
                    }
                    #endif
                    ImGui::EndMenu();
                  ...
                  if(ImGui::Selectable(filePaths[i].c_str(), false))
                    {
                        ...
                        else if(mode == DialogMode::OpenAnim)
                        {
                            openAnimationFile(filePaths[i]);
                        }
                        #ifdef CONVERT_FEATURE
                        else if(mode == DialogMode::ConvertToCPP)
                        {
       ...
       else if(mode == DialogMode::SaveAnimToFile)
       {
